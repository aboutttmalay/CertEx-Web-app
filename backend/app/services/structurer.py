...
def load_file_buffer(file_content: bytes, filename: str):
    ...
    ext = filename.split('.')[-1].lower() if filename and '.' in filename else 'unknown'
    try:
...
def clean_and_structure_data(df, column_mapping):
    ...
    for target_col, rules in column_mapping.items():
        ...
        if rule_type == 'email':
            series = series.apply(clean_email)
        elif rule_type == 'phone':
            series = series.apply(clean_phone)
        elif rule_type == 'currency':
            series = series.apply(clean_currency)
        elif rule_type == 'date':
            series = series.apply(clean_date)
        elif rule_type == 'text':
